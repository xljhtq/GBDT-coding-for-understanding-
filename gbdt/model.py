...
class GBDT:
    ...
    def fit(self, dataset, train_data):
        if self.loss_type == 'multi-classification':
            ...
            f = dict()  # 记录F_{m-1}的值
            self.loss.initialize(f, dataset)  # initialize F
            for iter in range(1, self.max_iter + 1):
                subset = train_data
                if 0 < self.sample_rate < 1:
                    subset = sample(subset, int(len(subset) * self.sample_rate))

                residual = self.loss.compute_residual(dataset, subset, f)  # 根据F_{m-1} , y的值来计算残差
                leaf_nodes = []
                targets = residual
                tree = construct_decision_tree(dataset, subset, targets, 0, leaf_nodes, self.max_depth, self.loss,
                                               self.split_points)
                self.trees[iter] = tree
                self.loss.update_f_value(f, tree, leaf_nodes, subset, dataset, self.learn_rate)  # 更新F值
