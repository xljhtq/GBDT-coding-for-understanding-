...
def FriedmanMSE(left_values, right_values):
    ...
    total_meal_left, total_meal_right = sum(left_values) / float(weighted_n_left), sum(right_values) / float(
        weighted_n_right)
...
def construct_decision_tree(dataset, remainedSet, targets, depth, leaf_nodes, max_depth, loss, criterion='MSE',
                            split_points=0):
    if depth < max_depth:
        ...
        node = LeafNode(remainedSet)             # LeftIdSet或RightIdSet的一些
